...
const QUERY =
  'query($q:String!,$p:Int!,$f:EmoteSearchFilter){emotes(query:$q,page:$p,limit:' + PER_PAGE +
...
export async function searchEmotes(query, page) {
  const body = {
    ...
    variables: {
      q: query, p: page,
      f: {
        category: 'TOP', exact_match: false, case_sensitive: false,
  ...
  return {
    count: d.count,
    items: d.items.map(e => ({
      id: e.id, name: e.name, animated: e.animated, files: e.host.files.map(f => f.name)
    }))
  };
}
